 ...
 * 更新论文的 FastGPT 同步状态
 * @param id 论文ID
 * @param collectionId FastGPT collection ID
 * @param syncSource 同步来源
 * @param hasFulltext 是否包含全文
 */
export async function updateFastgptSyncStatus(
  id: string,
  collectionId: string,
  ...
  try {
    const paper = await prisma.paper.update({
      where: { id },
      data: {
        fastgpt_collection_id: collectionId,
        fastgpt_sync_source: syncSource,
        fastgpt_sync_time: new Date(),
        fastgpt_has_fulltext: hasFulltext,
      },
    });
    logger.info("更新 FastGPT 同步状态成功", {
      id,
      collectionId,
      syncSource,
      hasFulltext,
    ...
    logger.error("更新 FastGPT 同步状态失败", { error: errorMessage, id });
    return null;
  }
}

/**
 * 批量更新论文的 FastGPT 同步状态
 * @param updates 更新列表
 */
export async function batchUpdateFastgptSyncStatus(
  updates: Array<{
    id: string;
    collectionId: string;
    syncSource: FastgptSyncSource;
  ...
  try {
    // 使用事务批量更新，保证原子性并减少数据库连接开销
    const results = await prisma.$transaction(
      updates.map((update) =>
        prisma.paper.update({
          where: { id: update.id },
          data: {
            fastgpt_collection_id: update.collectionId,
            fastgpt_sync_source: update.syncSource,
            fastgpt_sync_time: new Date(),
            fastgpt_has_fulltext: update.hasFulltext || false,
          },
        })
      )
    );
    logger.info("批量更新 FastGPT 同步状态成功", { count: results.length });
    return results;
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error("批量更新 FastGPT 同步状态失败", { error: errorMessage });
    return null;
  }
}
 ...
 * @param limit 查询数量限制
 */
export async function findPapersNotSyncedToFastgpt(limit: number = 100) {
  try {
    const papers = await prisma.paper.findMany({
      where: {
        fastgpt_collection_id: null,
        deleted_status: 0,
      },
      take: limit,
      orderBy: {
    ...
    logger.info("查询未同步到 FastGPT 的论文", { count: papers.length, limit });
    return papers;
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error("查询未同步到 FastGPT 的论文失败", { error: errorMessage, limit });
    return [];
  }
}

/**
 * 查询已同步但未包含全文的论文（用于全文补全）
 * @param limit 查询数量限制
 */
export async function findPapersSyncedWithoutFulltext(limit: number = 100) {
  try {
    const papers = await prisma.paper.findMany({
      where: {
        fastgpt_collection_id: { not: null },
        fastgpt_has_fulltext: false,
        pdf_downloaded: true, // 已下载 PDF
        deleted_status: 0,
      },
      take: limit,
      orderBy: {
    ...
    logger.info("查询已同步但未包含全文的论文", { count: papers.length, limit });
    return papers;
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error("查询已同步但未包含全文的论文失败", { error: errorMessage, limit });
    return [];
  }
}
